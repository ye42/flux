...
$classes = Flux::classes()
    ->add('group/listbox-button cursor-default p-[calc(0.5rem-1px)]')
...
<ui-pillbox-trigger {{ $attributes->class($classes) }} @if ($invalid) data-invalid @endif data-flux-group-target data-flux-pillbox-trigger>
    <?php if ($slot->isNotEmpty()): ?>
        {{ $slot }}
    <?php else: ?>
        <flux:pillbox.selected :$placeholder :$max :$suffix />
    <?php endif; ?>
    ...
    <flux:icon.chevron-down variant="mini" class="self-start mt-0.5 ms-2 me-px text-zinc-300 [[data-flux-pillbox-trigger]:hover_&]:text-zinc-800 [[disabled]_&]:text-zinc-200! dark:text-white/60 dark:[[data-flux-pillbox-trigger]:hover_&]:text-white dark:[[disabled]_&]:text-white/40!" />
</ui-pillbox-trigger>